<?php
// ============================================================
//  STUDENT PORTAL – GHANA VERSION (EMBEDDED REGIONS)
//  BLACK & GOLD THEME (NO GRADIENTS)
// ============================================================
...
// ---------- GHANA REGIONS (embedded as fallback) ----------
$defaultRegions = [
    ["region" => "Ashanti", "districts" => ["Kumasi", "Obuasi", "Odu", "Ejisu", "Mampong", "Konongo"]],
    ["region" => "Brong-Ahafo", "districts" => ["Sunyani", "Techiman", "Berekum", "Dormaa", "Nkoranza", "Wenchi"]],
    ["region" => "Central", "districts" => ["Cape Coast", "Sekondi-Takoradi", "Elmina", "Mfantsiman", "Abura", "Komenda"]],
    ["region" => "Eastern", "districts" => ["Koforidua", "Akwatia", "Nsawam", "Suhum", "Aburi", "Asamankese"]],
    ["region" => "Greater Accra", "districts" => ["Accra", "Tema", "Ga East", "Ga West", "Ashaiman", "Adenta"]],
    ["region" => "Northern", "districts" => ["Tamale", "Yendi", "Savelugu", "Gushiegu", "Tolon", "Karaga"]],
    ["region" => "Upper East", "districts" => ["Bolgatanga", "Bawku", "Navrongo", "Zuarungu", "Binduri"]],
    ["region" => "Upper West", "districts" => ["Wa", "Lawra", "Nandom", "Jirapa", "Lambussie"]],
    ["region" => "Volta", "districts" => ["Ho", "Keta", "Akatsi", "Hohoe", "Kpando", "Denu"]],
    ["region" => "Western", "districts" => ["Sekondi", "Tarkwa", "Axim", "Shama", "Nkroful", "Bibiani"]]
];

// Try to load from external JSON if present, otherwise use defaults
$jsonFile = 'ghana-regions.json';
...
// ============================================================
//  MERGED SQL SCHEMA + DATA (from the provided SQL dump)
// ============================================================
$sql = "
...
CREATE TABLE IF NOT EXISTS `students` (
  ...
  `jamb_score` int(11) DEFAULT 0,
  `profile_image` varchar(255) DEFAULT NULL,
  `admission_status` tinyint(4) DEFAULT 0 COMMENT '0=pending, 1=admitted, 2=rejected',
...
INSERT IGNORE INTO `students` (
  `id`, `first_name`, `middle_name`, `last_name`, `email`, `date_of_birth`,
  `gender`, `phone`, `address`, `state_of_origin`, `local_govt`,
  `next_of_kin`, `jamb_score`, `profile_image`, `admission_status`,
  `class`, `created_at`
) VALUES
...
// ---------- EXECUTE THE MERGED SQL IF NEEDED ----------
try {
    // Check if the table exists and how many rows it has
    ...
    if (!$tableExists) {
        ...
        if ($count < 130) {
            // Insert only the missing records (INSERT IGNORE will skip duplicates)
            $pdo->exec($sql);
        }
    }
} catch (PDOException $e) {
    // If the SQL fails, output the error for debugging (but you may want to log it instead)
...
function flashMessage() {
    if (isset($_SESSION['message'])) {
        ...
        return "<div class='message-banner $type'>$msg</div>";
    }
    return '';
}
...
if ($page === 'landing') {
    ...
    <div class="landing">
        <div class="card" style="max-width:600px; margin:60px auto;">
            <div class="header"><h1>🎓 Grace Academy Portal</h1><p>Check admission status</p></div>
            <div style="padding:2.5rem;">
                <div style="display:flex; gap:20px; justify-content:center; flex-wrap:wrap;">
                    <a href="?page=form" class="btn">➕ Register Student</a>
                    <a href="?page=dashboard" class="btn btn-secondary">📊 Dashboard</a>
                    <a href="?page=portal" class="btn btn-accent">🔍 Student Portal</a>
                </div>
            </div>
        </div>
    </div>
...
elseif ($page === 'form') {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $region = $_POST['state_of_origin'];
        $district = $_POST['local_govt'];
        $next_of_kin = trim($_POST['next_of_kin']);
        $jamb_score = intval($_POST['jamb_score']);
        $class = $_POST['class'];

        $errors = [];
        if (strlen($first_name) < 2 || !preg_match('/^[a-zA-Z\s.]+$/', $first_name)) $errors[] = "Valid first name required";
        if (strlen($last_name) < 2 || !preg_match('/^[a-zA-Z\s.]+$/', $last_name)) $errors[] = "Valid last name required";
        if (!filter_var($email, FILTER_VALIDATE_EMAIL)) $errors[] = "Valid email required";
        if (!$gender) $errors[] = "Gender required";
        if (!$region) $errors[] = "Region required";
        if (!$district) $errors[] = "District required";
        if (!$class) $errors[] = "Class required";
        ...
        if (empty($errors)) {
            try {
                $stmt = $pdo->prepare("INSERT INTO students (first_name, middle_name, last_name, email, gender, phone, date_of_birth, address, state_of_origin, local_govt, next_of_kin, jamb_score, profile_image, class)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                $stmt->execute([$first_name, $middle_name, $last_name, $email, $gender, $phone, $dob, $address, $region, $district, $next_of_kin, $jamb_score, $profile_image, $class]);
    ...
    foreach ($regions as $r) {
        $regionOptions .= '<option value="' . escapeHtml($r['region']) . '">' . escapeHtml($r['region']) . '</option>';
    }
    ...
    <div class="container">
        <div class="card">
            <div class="header flex-between">
                ...
                <div class="nav">
                    <a href="?page=landing" class="nav-btn">🏠 Home</a>
                    <a href="?page=dashboard" class="nav-btn">📊 Dashboard</a>
                    <a href="?page=portal" class="nav-btn">🔍 Portal</a>
                </div>
            </div>
            <?= flashMessage() ?>
            <form method="POST" enctype="multipart/form-data">
                <div class="form-grid">
                    <div class="field-group"><label>First Name *</label><input type="text" name="first_name" required></div>
                    <div class="field-group"><label>Middle Name</label><input type="text" name="middle_name"></div>
                    <div class="field-group"><label>Last Name *</label><input type="text" name="last_name" required></div>
                    <div class="field-group"><label>Email *</label><input type="email" name="email" required></div>
                    <div class="field-group"><label>Date of Birth</label><input type="date" name="date_of_birth"></div>
                    <div class="field-group"><label>Gender *</label><select name="gender" required><option value="">Select</option><option>Male</option><option>Female</option><option>Other</option></select></div>
                    <div class="field-group"><label>Phone</label><input type="tel" name="phone"></div>
                    <div class="field-group"><label>Address</label><textarea name="address" rows="2"></textarea></div>
                    <div class="field-group"><label>Class *</label>
                    ...
                    <div class="field-group"><label>Region *</label><select name="state_of_origin" id="region" required><?= $regionOptions ?></select></div>
                    <div class="field-group"><label>District *</label><select name="local_govt" id="district" required disabled><option value="">Select region first</option></select></div>
                    <div class="field-group"><label>Next of Kin</label><input type="text" name="next_of_kin"></div>
                    <div class="field-group"><label>JAMB Score</label><input type="number" name="jamb_score" min="0" max="400"></div>
                    <div class="field-group"><label>Profile Image</label><input type="file" name="profile_image" accept="image/*"><div class="image-preview" id="preview" style="display:none;"><img id="previewImg"></div></div>
                </div>
                <div class="form-actions"><button type="submit">✅ Register Student</button></div>
            </form>
            <footer><a href="?page=dashboard">Dashboard</a> | <a href="?page=landing">Home</a></footer>
        </div>
    </div>
    <script>
        const regionsData = <?= json_encode($regions) ?>;
        const regionSelect = document.getElementById('region');
        const districtSelect = document.getElementById('district');
        regionSelect.addEventListener('change', function() {
            const selectedRegion = this.value;
            districtSelect.innerHTML = '<option value="">-- Select District --</option>';
            if (!selectedRegion) { districtSelect.disabled = true; return; }
            const regionObj = regionsData.find(r => r.region === selectedRegion);
...
elseif ($page === 'dashboard') {
    ...
    if (!empty($_GET['name'])) {
        $conditions[] = "CONCAT(first_name, ' ', COALESCE(middle_name, ''), ' ', last_name) LIKE :name";
        $params['name'] = '%' . $_GET['name'] . '%';
    }
    if (isset($_GET['admission_status']) && $_GET['admission_status'] !== '') {
        $conditions[] = "admission_status = :admission_status";
        $params['admission_status'] = (int)$_GET['admission_status'];
    }
    ...
    if (!empty($_GET['jamb_min']) || !empty($_GET['jamb_max'])) {
        $min = intval($_GET['jamb_min'] ?? 0);
        $max = intval($_GET['jamb_max'] ?? 400);
        $conditions[] = "jamb_score BETWEEN :jamb_min AND :jamb_max";
        $params['jamb_min'] = $min;
        $params['jamb_max'] = $max;
    }

    $where = $conditions ? "WHERE " . implode(" AND ", $conditions) : "";
    $sql = "SELECT * FROM students $where ORDER BY created_at DESC";
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_status'])) {
        ...
        $newStatus = intval($_POST['admission_status']);
        $upd = $pdo->prepare("UPDATE students SET admission_status = ? WHERE id = ?");
        $upd->execute([$newStatus, $id]);
        header("Location: ?page=dashboard");
        exit;
    }

    // ---- AJAX request: return only the table rows ----
    if (isset($_GET['ajax']) && $_GET['ajax'] == 1) {
        foreach ($students as $s) {
            ?>
            <tr>
                <td><img class="profile-thumb" src="<?= escapeHtml($s['profile_image'] ?: 'placeholder.jpg') ?>" onerror="this.src='placeholder.jpg'"></td>
                ...
                <td><?= nl2br(escapeHtml($s['address'])) ?></td>
                <td><?= escapeHtml($s['state_of_origin']) ?></td>
                <td><?= escapeHtml($s['local_govt']) ?></td>
                <td><?= escapeHtml($s['next_of_kin']) ?></td>
                <td><?= $s['jamb_score'] ?></td>
                <td>
                    <span class="status-badge <?= statusBadge($s['admission_status']) ?>">
                        <?= statusIcon($s['admission_status']) ?>
                    </span>
                </td>
                <td><a href="?page=view&id=<?= $s['id'] ?>" class="view-btn">👁️ View</a></td>
            </tr>
            <?php
        }
        exit;
    ...
    <div class="container">
        <div class="card">
            <div class="header flex-between">
                <h1>📊 Student Dashboard (<?= count($students) ?>)</h1>
                <div class="nav">
                    <a href="?page=landing" class="nav-btn">🏠 Home</a>
                    <a href="?page=form" class="nav-btn">➕ New</a>
                    <a href="?page=portal" class="nav-btn">🔍 Portal</a>
                </div>
            </div>
            <div class="filter-bar">
                <form method="GET" action="?page=dashboard" id="filterForm">
                    <input type="hidden" name="page" value="dashboard">
                    <div class="filter-group">
                        <input type="text" name="name" id="nameSearch" placeholder="🔍 Search by name" value="<?= escapeHtml($_GET['name'] ?? '') ?>" autocomplete="off">
                    </div>
                    <div class="filter-group">
                        <select name="admission_status" id="statusFilter">
                            <option value="">All status</option>
                            <option value="1" <?= isset($_GET['admission_status']) && $_GET['admission_status'] == '1' ? 'selected' : '' ?>>Admitted</option>
                            <option value="2" <?= isset($_GET['admission_status']) && $_GET['admission_status'] == '2' ? 'selected' : '' ?>>Declined</option>
                    ...
                    <div class="filter-group">
                        <select name="class" id="classFilter">
                            ...
                            <?php foreach ($classFilterOptions as $cls): ?>
                                <option value="<?= $cls ?>" <?= isset($_GET['class']) && $_GET['class'] == $cls ? 'selected' : '' ?>><?= $cls ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="filter-group">
                        ...
                        <div style="display:flex; gap:4px;">
                            <input type="number" name="jamb_min" id="jambMin" placeholder="Min" value="<?= escapeHtml($_GET['jamb_min'] ?? '') ?>" style="width:70px;">
                            <input type="number" name="jamb_max" id="jambMax" placeholder="Max" value="<?= escapeHtml($_GET['jamb_max'] ?? '') ?>" style="width:70px;">
                        </div>
                    </div>
                    <div class="filter-group">
                        <button type="submit">Apply Filters</button>
                        <a href="?page=dashboard" class="btn-reset">Reset</a>
                    </div>
                </form>
            </div>
    ...
    <script>
        function fetchFilteredRows() {
            ...
            .then(html => {
                ...
                document.querySelector('.header h1').textContent = `📊 Student Dashboard (${count})`;
            })
            .catch(err => console.error('AJAX error:', err));
        }

        document.addEventListener('DOMContentLoaded', function() {
            document.getElementById('nameSearch').addEventListener('input', fetchFilteredRows);
            document.getElementById('statusFilter').addEventListener('change', fetchFilteredRows);
            document.getElementById('genderFilter').addEventListener('change', fetchFilteredRows);
            document.getElementById('classFilter').addEventListener('change', fetchFilteredRows);
            document.getElementById('jambMin').addEventListener('change', fetchFilteredRows);
            document.getElementById('jambMax').addEventListener('change', fetchFilteredRows);
...
elseif ($page === 'view') {
    ...
    if (!$student) { header("Location: ?page=dashboard"); exit; }

    // ---- Handle status update via checkbox or decline button ----
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        if (isset($_POST['toggle_admitted'])) {
            // Checkbox: if checked -> admitted (1), else pending (0)
            $newStatus = isset($_POST['admitted']) ? 1 : 0;
        ...
        if (isset($newStatus)) {
            $upd = $pdo->prepare("UPDATE students SET admission_status = ? WHERE id = ?");
            $upd->execute([$newStatus, $id]);
            header("Location: ?page=view&id=$id&updated=1");
            exit;
        }
    }

    $currentStatus = $student['admission_status'];
    $isAdmitted = ($currentStatus == 1);
    $isDeclined = ($currentStatus == 2);
    ?>
    <div class="container">
        <div class="card">
            <div class="header flex-between">
                <h1>👤 <?= escapeHtml($student['first_name'] . ' ' . ($student['middle_name'] ?? '') . ' ' . $student['last_name']) ?> – Profile</h1>
                <div class="nav">
                    <a href="?page=dashboard" class="nav-btn">📊 Dashboard</a>
                    <a href="?page=landing" class="nav-btn">🏠 Home</a>
                </div>
            ...
            <?php if (isset($_GET['updated'])): ?>
                <div class="message-banner success">✅ Admission status updated!</div>
            <?php endif; ?>
            <div class="profile-layout">
                <div class="profile-avatar">
                    <img src="<?= escapeHtml($student['profile_image'] ?: 'placeholder.jpg') ?>" onerror="this.src='placeholder.jpg'">
                </div>
                <div class="profile-info">
                    <div class="info-section">
                        <h3>Personal Information</h3>
                        <div class="info-row"><span class="info-label">First Name:</span> <?= escapeHtml($student['first_name']) ?></div>
                        <div class="info-row"><span class="info-label">Middle Name:</span> <?= escapeHtml($student['middle_name'] ?? '') ?></div>
                        <div class="info-row"><span class="info-label">Last Name:</span> <?= escapeHtml($student['last_name']) ?></div>
                        <div class="info-row"><span class="info-label">Email:</span> <?= escapeHtml($student['email']) ?></div>
                        <div class="info-row"><span class="info-label">Gender:</span> <?= escapeHtml($student['gender']) ?></div>
                        <div class="info-row"><span class="info-label">Phone Number:</span> <?= escapeHtml($student['phone']) ?></div>
                        <div class="info-row"><span class="info-label">Date Of Birth:</span> <?= $student['date_of_birth'] ?></div>
                        <div class="info-row"><span class="info-label">Address:</span> <?= nl2br(escapeHtml($student['address'])) ?></div>
                        <div class="info-row"><span class="info-label">Class:</span> <?= escapeHtml($student['class'] ?? 'N/A') ?></div>
                    </div>
                    <div class="info-section">
                        <h3>Other Information</h3>
                        <div class="info-row"><span class="info-label">State Of Origin:</span> <?= escapeHtml($student['state_of_origin']) ?></div>
                        <div class="info-row"><span class="info-label">Local Govt:</span> <?= escapeHtml($student['local_govt']) ?></div>
                        <div class="info-row"><span class="info-label">Next Of Kin:</span> <?= escapeHtml($student['next_of_kin']) ?></div>
                    </div>
                    <div class="info-section">
                        <h3>Academics & Admission</h3>
                        <div class="info-row"><span class="info-label">JAMB Score:</span> <?= $student['jamb_score'] ?></div>
                        <div class="info-row"><span class="info-label">Current Status:</span>
                            <span class="status-badge <?= statusBadge($currentStatus) ?>">
                        ...
                        <div class="info-row">
                            ...
                            <form method="POST" style="display:flex; gap:10px; align-items:center; flex-wrap:wrap;">
                                ...
                                <button type="submit" name="toggle_admitted" class="btn-sm">Update</button>
                                <!-- Decline button -->
                                <button type="submit" name="decline" class="btn-sm btn-danger" <?= $isDeclined ? 'disabled' : '' ?>>Decline</button>
                                <span style="font-size:0.8rem; color:#666;">(Check = Admitted, Uncheck = Pending)</span>
                            </form>
                        </div>
                    </div>
                </div>
            </div>
            <footer><a href="?page=dashboard">← Back to Dashboard</a></footer>
        </div>
    </div>
    <?php
}
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['check_email'])) {
        ...
        if ($student) {
            ...
            <div class="student-card">
                <img class="portal-avatar" src="'.escapeHtml($student['profile_image'] ?: 'placeholder.jpg').'" onerror="this.src=\'placeholder.jpg\'">
                <div class="student-details">
                    <h3>'.escapeHtml($student['first_name']).' '.escapeHtml($student['middle_name'] ?? '').' '.escapeHtml($student['last_name']).'</h3>
                    <div><strong>Email:</strong> '.escapeHtml($student['email']).'</div>
                    <div><strong>Class:</strong> '.escapeHtml($student['class'] ?? 'N/A').'</div>
                    <div><strong>Region:</strong> '.escapeHtml($student['state_of_origin']).'</div>
                    <div><strong>District:</strong> '.escapeHtml($student['local_govt']).'</div>
                    <div><strong>JAMB Score:</strong> '.$student['jamb_score'].'</div>
                    <div><strong>Admission Status:</strong> <span class="status-badge '.statusBadge($student['admission_status']).'">'.statusIcon($student['admission_status']).'</span></div>
                </div>
            </div>';
        } else {
            $result = '<div class="message-banner error" style="display:block;">❌ No student found with that email.</div>';
        }
    }
    ?>
    <div class="container">
        <div class="card">
            <div class="header flex-between">
                <h1>🔍 Student Portal – Check Admission Status</h1>
                <div class="nav">
                    <a href="?page=landing" class="nav-btn">🏠 Home</a>
                    <a href="?page=dashboard" class="nav-btn">📊 Dashboard</a>
                </div>
            ...
            <div style="padding:2rem;">
                <form method="POST">
                    <div style="display:flex; gap:12px; max-width:500px;">
                        <input type="email" name="email" placeholder="Enter your registered email" required style="flex:1;">
                        <button type="submit" name="check_email">Check Status</button>
                    </div>
                </form>
                <div style="margin-top:1.5rem;"><?= $result ?></div>
            </div>
            <footer>Contact admission office for any enquiries.</footer>
        </div>
    </div>
    <?php
}
...
<head>
    ...
    <title>Student Portal – Ghana</title>
    <style>
        /* ========== BLACK & GOLD THEME (NO GRADIENTS) ========== */
        * { margin:0; padding:0; box-sizing:border-box; font-family:system-ui, -apple-system, sans-serif; }
        ...
        .card {
            background: #ffffff;
            border-radius: 32px;
            box-shadow: 0 20px 35px -12px rgba(255,215,0,0.3);
            overflow-x:auto;
            margin-bottom:24px;
        ...
        .form-grid { display:grid; grid-template-columns:repeat(auto-fit,minmax(280px,1fr)); gap:1.2rem; padding:2rem; }
        ...
        .form-actions { padding:0 2rem 2rem; }
        button {
            background: #000000;
            color: #fdd835;
            border: 2px solid #fdd835;
        ...
        .message-banner.error { background:#fff5f5; border-left-color:#e53e3e; color:#742a2a; }
        .message-banner.success { background:#e6fffa; border-left-color:#38a169; color:#22543d; }
        .filter-bar { margin:20px 24px; }
        .filter-bar form { display:flex; flex-wrap:wrap; gap:12px; align-items:flex-end; }
        .filter-group { display:flex; flex-direction:column; gap:4px; }
        .filter-group label { font-size:0.8rem; font-weight:600; color:#000000; }
        .filter-group input, .filter-group select { padding:8px 12px; border-radius:40px; }
        ...
        .table-wrapper { overflow-x:auto; padding:0 1rem 1.5rem; }
        ...
        .profile-thumb { width:40px; height:40px; border-radius:50%; object-fit:cover; background:#e2e8f0; }
        .status-badge { display:inline-block; padding:4px 10px; border-radius:40px; font-size:0.7rem; font-weight:700; white-space:nowrap; }
        .status-admitted { background:#c6f6d5; color:#22543d; }
        .status-declined { background:#fed7d7; color:#9b2c2c; }
        .status-pending { background:#feebc8; color:#9b6f4b; }
        .view-btn {
            background: #000000;
            color: #fdd835;
            padding:5px 12px;
        ...
        .image-preview { margin-top:8px; width:70px; height:70px; border-radius:16px; overflow:hidden; background:#f1f5f9; border:1px solid #cbd5e1; }
        .image-preview img { width:100%; height:100%; object-fit:cover; }
        .landing { max-width:600px; margin:60px auto; text-align:center; }
        .landing .buttons { display:flex; gap:20px; justify-content:center; margin-top:30px; flex-wrap:wrap; }
        .profile-layout {
            display: flex;
            gap: 2rem;
            padding: 2rem;
        ...
        .info-row select, .info-row input[type="checkbox"] {
            padding: 4px 10px;
            border-radius: 20px;
            border: 1.5px solid #fdd835;
            background: #ffffff;
        ...
        .btn-sm:hover { background: #fdd835; color: #000000; }
        .btn-danger { background: #b91c1c; color: white; border-color: #b91c1c; }
        .btn-danger:hover { background: #fdd835; color: #b91c1c; border-color: #b91c1c; }
        .student-card { background:#ffffff; border-radius:28px; padding:1.5rem; display:flex; flex-wrap:wrap; gap:1.5rem; align-items:center; border:2px solid #fdd835; }
        .portal-avatar { width:100px; height:100px; border-radius:50%; object-fit:cover; border:3px solid #000000; }
        .student-details { flex:1; }
        footer { text-align:center; padding:20px; color:#fff; }
        footer a { color:#fdd835; text-decoration:none; font-weight:bold; }
